...
void mag3110::configMag(void) {
  ...
  Wire.send(0x79); // Active Mode, 1.25 Hz datarate, 8x oversampling
  ...
  Wire.send(0x80);        // Auto resets before each measurement
  Wire.endTransmission();          // stop transmitting
}
...
int mag3110::readTemp(void)
{
  int t;  //define the MSB and LSB

  Wire.beginTransmission(MAG_ADDR); // transmit to device 0x0E
  Wire.send(0x0f);              // z MSB reg
  Wire.endTransmission();       // stop transmitting
  ...
  while(Wire.available())    // slave may send less than requested
  {
    t = Wire.receive(); // receive the byte
  }
  return t;
}
